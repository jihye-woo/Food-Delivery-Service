...
if($_SESSION['$selected_qid']!=NULL){ 
    ...
    $query = "select qid, rname, location, min_price, lid from WAITLIST Natural Join RESTAURANT Natural Join LOCATION where waitlist.qid = $selected_qid;";
    $result = $db->query($query);
    if($result != null){
        $row = $result->fetch_assoc();
        $selected_items = array($row['qid'], $row['rname'], $row['location'] ,$row['min_price']);
...
<section>
    ...
    <form method = 'POST' name = 'select_num_of_menu' action = '<?php $_SESSION['menu']  = $menu_num; ?>'>
        <table>
            ...
            if($_SESSION['$selected_qid']!=NULL){
                $selected_qid =$_SESSION['$selected_qid']; 
                $q = "select DISTINCT oid, menu, price from menu Natural Join Order_queue Natural Join Orders where qid = $selected_qid;";
                $result = $db -> query($q);

                $orders_info = array();

                while($row = $result -> fetch_assoc()){
                    $orders_info[$row['oid']]  = array($row['menu'], $row['price'], 0);
                }
